...
class TwitterAPICaller: BDBOAuth1SessionManager {    
    ...
    func favoriteTweet(tweetId:Int, success: @escaping () -> (), failute: @escaping (Error) -> ()) {
        ...
        TwitterAPICaller.client?.post(url, parameters: ["id": tweetId], progress: nil, success: { (task: URLSessionDataTask, response: Any?) in
            success()
        }, failure: { (task: URLSessionDataTask?, error: Error) in failute(error)
        })
  }

    func unfavoriteTweet(tweetId:Int, success: @escaping () -> (), failute: @escaping (Error) -> ()) {
    ...
    func retweet(tweetId:Int, success: @escaping () -> (), failute: @escaping (Error) -> ()) {
